...
def validate(model,dataloader,accelerator):
    ...
    return sum(ranks)/len(ranks),orig_loss
...
def main():
    ...
    accelerator.init_trackers(
        project_name="windowh1/knowledge-distillation", 
        config=args,
    )
        ...
        for step,batch in enumerate(train_dataloader):
            with accelerator.accumulate(dual_encoder):
                with accelerator.autocast():
                    ...
                    loss = calculate_orig_loss(matching_score,labels=labels)
                ...
                if accelerator.sync_gradients:
                    ...
                    if completed_steps % EVAL_STEPS == 0:
                        avg_rank,orig_loss = validate(dual_encoder,dev_dataloader,accelerator)
                        
                        dual_encoder.train()
                        accelerator.log({"avg_rank": avg_rank, "orig_loss":orig_loss}, step=completed_steps)
