...
class LargeScoreCalculator(object):
    def __init__(self):
        self.dpr_query_encoder = DPRQuestionEncoder.from_pretrained("facebook/dpr-question_encoder-multiset-base")
        self.dpr_doc_encoder = DPRContextEncoder.from_pretrained("facebook/dpr-ctx_encoder-multiset-base")
        self.lex_query_encoder = AutoModel.from_pretrained('facebook/spar-wiki-bm25-lexmodel-query-encoder')
        self.lex_doc_encoder = AutoModel.from_pretrained('facebook/spar-wiki-bm25-lexmodel-context-encoder')
    
    def get_large_score(self, batch, concat_weight=0.7):
        with torch.no_grad():
            query_input = batch['query_input_ids']
            query_attention_mask = batch['query_attention_mask']
            query_token_type_ids = batch['query_token_type_ids']

            doc_input = batch['doc_input_ids']
            doc_attention_mask = batch['doc_attention_mask']
            doc_token_type_ids = batch['doc_token_type_ids']

            dpr_query_emb = self.dpr_query_encoder(input_ids=query_input, attention_mask=query_attention_mask, token_type_ids=query_token_type_ids).pooler_output
                # [batch_size, hidden_size] = [16, 768]
            dpr_doc_emb = self.dpr_doc_encoder(input_ids=doc_input, attention_mask=doc_attention_mask, token_type_ids=doc_token_type_ids).pooler_output
                # [batch_size, hidden_size] = [32, 768]
            lex_query_emb = self.lex_query_encoder(input_ids=query_input, attention_mask=query_attention_mask, token_type_ids=query_token_type_ids).pooler_output
                # [batch_size, hidden_size] = [16, 768]
            lex_doc_emb = self.lex_doc_encoder(input_ids=doc_input, attention_mask=doc_attention_mask, token_type_ids=doc_token_type_ids).pooler_output
                # [batch_size, hidden_size] = [32, 768]
            large_score = torch.matmul(dpr_query_emb, dpr_doc_emb.permute(1,0)) + concat_weight * torch.matmul(lex_query_emb, lex_doc_emb.permute(1,0)) # [16, 32]
...
def validate(model, spar_model, dataloader, accelerator):
    model.eval()
    query_embeddings = []
...
def main():
    ...
    dual_encoder.train()
    calculator = LargeScoreCalculator()
        ...
        for step,batch in enumerate(train_dataloader):
            with accelerator.accumulate(dual_encoder):
                with accelerator.autocast():
                    ...
                    large_score = calculator.get_large_score(batch).to(matching_score.device)
